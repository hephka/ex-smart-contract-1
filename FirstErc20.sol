...
contract FirstErc20 {
    ...
    // mapping from addresses that check if it is an administrator.
    ...
    // Address of the contract owner defined as fisrt admin.
    address private _firstAdmin;
    ...
    modifier onlyAdmin() {
        require(_admins[msg.sender], "ERC20: Only owner can perform this action");
        _;
    }
    ...
    // Returns the address from admins.
    function admins(address _account) public view returns (bool) {
    ...
    // Burns `_amount` tokens and assigns them to `_account`, increasing
    // the total supply.
    // Emits a `Transfer` event with `_from` set to the zero address.
